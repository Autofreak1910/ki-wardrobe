...
function QuestionReveal({
  ...
  return (
    <div style={{ width: '100%' }}>
      <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: '22px', fontWeight: 500, color: text, marginBottom: '18px', letterSpacing: '-0.02em', textAlign: 'center' as const, lineHeight: 1.3 }}>
        {question}
      </h2>

      {!answer && (
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' as const, marginBottom: '20px' }}>
          {options.map((opt) => (
            <motion.button key={opt} whileTap={{ scale: 0.95 }}
              onClick={() => setAnswer(opt)}
              style={{ padding: '12px 22px', borderRadius: '100px', border: `1.5px solid ${accent}`, background: accentDim, color: accent, fontSize: '14px', fontWeight: 700, cursor: 'pointer', fontFamily: "'Poppins', 'Inter', sans-serif" }}>
              {opt}
            </motion.button>
          ))}
        </div>
          ...
          {index === 0 && (
            <motion.div key={0} drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.6} onDragEnd={handleDragEnd}
              ...
              <motion.p
                ...
                style={{ fontSize: '56px', marginBottom: '18px' }}
              >
                🪄
              </motion.p>
              <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: '27px', fontWeight: 500, color: text, marginBottom: '14px', letterSpacing: '-0.03em', lineHeight: 1.25 }}>
                {isDe
          ...
          {index === 1 && (
            <motion.div key={1} drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.6} onDragEnd={handleDragEnd}
              ...
              <QuestionReveal
                ...
                text={text} muted={muted} accent={accent} accentDim={accentDim} card={card} border={border} isDark={isDark}
              >
