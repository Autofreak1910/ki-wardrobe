...
export default function CookieConsent() {
  ...
  useEffect(() => {
    const consent = localStorage.getItem('kw_cookie_consent_v5')
    if (consent === 'accepted') {
      loadGoogleAnalytics()
    } else if (!consent) {
  ...
  function accept() {
  localStorage.setItem('kw_cookie_consent_v5', 'accepted')
    loadGoogleAnalytics()
    setShow(false)
  }

  function decline() {
  localStorage.setItem('kw_cookie_consent_v5', 'declined')
    setShow(false)
  }
