...
export default function CookieConsent() {
  ...
  useEffect(() => {
    const consent = localStorage.getItem('kw_cookie_consent_v4')
    if (consent === 'accepted') {
      loadGoogleAnalytics()
    } else if (!consent) {
  ...
  function accept() {
  localStorage.setItem('kw_cookie_consent_v4', 'accepted')
    loadGoogleAnalytics()
    setShow(false)
  }

  function decline() {
  localStorage.setItem('kw_cookie_consent_v4', 'declined')
    setShow(false)
  }
