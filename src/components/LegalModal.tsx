...
type TabKey = 'impressum' | 'datenschutz' | 'agb'

export default function LegalModal({ open, onClose, initialTab = 'impressum' }: { open: boolean; onClose: () => void; initialTab?: TabKey }) {
  ...
  const sections: Record<TabKey, { heading: string; items: { title: string; content: string }[] }> = {
    impressum: {
      heading: 'Impressum',
      items: [
        { title: 'Angaben gemäß § 5 TMG', content: 'Luca Darvas\nBernd-Rosemeyer-Straße 14\n85551 Kirchheim bei München\nDeutschland' },
        { title: 'Kontakt', content: 'E-Mail: support@kiwardrobe.com' },
    ...
    datenschutz: {
      ...
      items: [
        ...
        { title: 'Feedback', content: 'Wenn du uns über die Feedback-Funktion in der App eine Nachricht schickst, speichern wir den Nachrichtentext, die von dir gewählte Kategorie (Feedback, Feature-Idee, Bug, Sonstiges), eine optionale Sternebewertung sowie — falls du eine Antwort wünschst — deine Kontakt-E-Mail. Diese Daten nutzen wir ausschließlich, um dein Anliegen zu bearbeiten und dir bei Bedarf zu antworten.' },
        ...
        { title: 'Datenlöschung', content: 'Du kannst dein Konto inklusive aller gespeicherten Daten (Profil, Kleidungsstücke, Outfits, Avatar-Bilder, Push-Anmeldungen) jederzeit selbst über die App löschen (Profil → Account löschen). Diese Löschung ist sofort wirksam und unwiderruflich. Alternativ kannst du uns auch unter support@kiwardrobe.com kontaktieren.' },
    ...
    agb: {
      ...
      items: [
        ...
        { title: '5. Laufzeit und Kündigung', content: 'Das Pro-Abonnement verlängert sich automatisch um jeweils einen Monat, sofern es nicht rechtzeitig vor Ablauf des laufenden Abrechnungszeitraums gekündigt wird. Die Kündigung ist jederzeit selbstständig über den Button "Abo verwalten / kündigen" in den Profileinstellungen möglich — dieser öffnet das sichere Kundenportal unseres Zahlungsdienstleisters Stripe, in dem du dein Abonnement direkt kündigen oder deine Zahlungsdaten verwalten kannst. Alternativ ist die Kündigung auch per E-Mail an support@kiwardrobe.com möglich. Bereits bezahlte Zeiträume werden bei einer Kündigung nicht anteilig zurückerstattet; der Zugang zu Pro-Funktionen bleibt bis zum Ende des bezahlten Zeitraums bestehen.' },
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <button onClick={onClose} style={{ background: card, border: `1px solid ${border}`, borderRadius: '10px', width: '32px', height: '32px', cursor: 'pointer', fontSize: '14px', color: muted, flexShrink: 0 }}>✕</button>
            </div>

            {/* Tab Switch */}
            <div style={{ display: 'flex', gap: '8px', marginBottom: '18px', background: secondary, borderRadius: '12px', padding: '4px', position: 'relative' as const }}>
              {(['impressum', 'datenschutz', 'agb'] as const).map(t => (
                <button key={t} onClick={() => setTab(t)}
                  style={{ flex: 1, position: 'relative' as const, padding: '10px', borderRadius: '10px', border: 'none', background: 'transparent', color: tab === t ? text : muted, fontSize: '12px', fontWeight: tab === t ? 600 : 400, cursor: 'pointer', fontFamily: "'Poppins', 'Inter', sans-serif", zIndex: 1 }}>
                  {tab === t && (
                    <motion.div layoutId="legalModalTabPill" transition={{ type: 'spring', duration: 0.4, bounce: 0.15 }}
                      style={{ position: 'absolute' as const, inset: 0, background: card, borderRadius: '10px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', zIndex: -1 }} />
                  )}
            ...
            <AnimatePresence mode="wait">
              <motion.div key={tab}
                ...
                style={{ background: card, border: `1px solid ${border}`, borderRadius: '16px', padding: '20px' }}>
                <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: '20px', fontWeight: 500, color: text, marginBottom: '16px' }}>{active.heading}</h1>

                {active.items.map(section => (
                  <div key={section.title} style={{ marginBottom: '18px' }}>
                    <h3 style={{ fontSize: '12px', fontWeight: 600, color: muted, textTransform: 'uppercase' as const, letterSpacing: '0.08em', marginBottom: '6px' }}>{section.title}</h3>
                    <p style={{ fontSize: '13.5px', color: text, lineHeight: 1.7, whiteSpace: 'pre-line' as const }}>{section.content}</p>
                  </div>
                ))}
              </motion.div>
            </AnimatePresence>
