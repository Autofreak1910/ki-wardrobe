...
useEffect(() => {
  const timer = setTimeout(() => {
    try {
     if (localStorage.getItem('kw_rating_done_v3')) return
      const laterUntil = localStorage.getItem('kw_rating_later_v3')
        ...
        {showStreakInfo && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }}
              ...
              onClick={e => e.stopPropagation()}
              style={{ background: card, borderRadius: '28px', width: '100%', maxWidth: '400px', border: `1px solid ${border}`, maxHeight: 'calc(100dvh - 190px - env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column' as const, position: 'relative' as const, overflow: 'hidden' }}>
