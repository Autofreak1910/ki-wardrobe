...
const searchParams = useSearchParams()
  ...
  const sections: Record<'impressum' | 'datenschutz' | 'agb', { heading: string; items: { title: string; content: string }[] }> = {
    impressum: {
      heading: 'Impressum',
      items: [
  ...
  <div className="app-container" style={{ height: '100vh', display: 'flex', flexDirection: 'column' as const, background: bg, fontFamily: "'Poppins', 'Inter', sans-serif", overflow: 'hidden' }}>
      <motion.main initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
        ...
        <motion.button initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05, duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
          ...
          onClick={() => router.back()} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px', color: muted, fontFamily: "'Poppins', 'Inter', sans-serif", padding: '0', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '6px' }}>
          ← {locale === 'de' ? 'Zurück' : 'Back'}
        </motion.button>

        {/* Tab Switch */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08, duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
          style={{ display: 'flex', gap: '8px', marginBottom: '24px', background: secondary, borderRadius: '12px', padding: '4px', position: 'relative' as const }}>
          {(['impressum', 'datenschutz', 'agb'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {tab === t && (
                <motion.div layoutId="legalTabPill" transition={{ type: 'spring', duration: 0.4, bounce: 0.15 }}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={tab}
            ...
            style={{ background: card, border: `1px solid ${border}`, borderRadius: '16px', padding: '24px' }}>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: '24px', fontWeight: 500, color: text, marginBottom: '20px' }}>{active.heading}</h1>

            {active.items.map((section, i) => (
              <motion.div key={section.title}
                initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: Math.min(i * 0.03, 0.24), duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
