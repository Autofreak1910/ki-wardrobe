...
if (profileRes.data) { setProfile({ ...profileRes.data, email: session.user.email }); setEditUsername(profileRes.data.username ?? '') }
  ...
  const FREE_LIMITS = { outfits: 3, items: 20, saved: 5 }
  const PRO_LIMITS = { outfits: 15, items: '∞', saved: '∞' }
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                <h1 style={{ fontSize: '20px', fontWeight: 800, color: text, letterSpacing: '-0.03em' }}>
                  {profile?.username ?? 'User'}
                </h1>
                {isPremium ? (
                  <span style={{ fontSize: '10px', fontWeight: 700, color: '#fff', background: `linear-gradient(135deg, ${accent}, #6b9fff)`, borderRadius: '6px', padding: '3px 8px', letterSpacing: '0.05em' }}>PRO ✦</span>
                ) : (
                  <span style={{ fontSize: '10px', fontWeight: 700, color: muted, background: accentDim, border: `1px solid ${border}`, borderRadius: '6px', padding: '3px 8px', letterSpacing: '0.05em' }}>FREE</span>
                )}
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
            {[
              ...
              { label: locale === 'de' ? 'Outfits heute' : 'Outfits today', value: todayOutfits, max: isPremium ? null : 3 },
          ...
          <motion.div
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                <div>
                  <p style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.7)', letterSpacing: '0.08em', textTransform: 'uppercase' as const, marginBottom: '4px' }}>
                    {locale === 'de' ? 'Upgrade' : 'Upgrade'}
                  </p>
                  <h2 style={{ fontSize: '20px', fontWeight: 800, color: '#fff', letterSpacing: '-0.03em', fontFamily: "'DM Sans', sans-serif" }}>
                    KiWardrobe Pro ✦
                  </h2>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '16px' }}>
                {[
                  ...
                  { free: '5', pro: '∞', label: locale === 'de' ? 'Outfits speichern' : 'Saved outfits' },
              ...
              <div style={{ background: 'rgba(255,255,255,0.2)', borderRadius: '12px', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <p style={{ fontSize: '13px', fontWeight: 700, color: '#fff' }}>
                  {locale === 'de' ? 'Jetzt upgraden →' : 'Upgrade now →'}
                </p>
      ...
      <button onClick={() => {
        ...
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: '16px', overflow: 'hidden', marginBottom: '24px' }}>
          <button onClick={handleLogout}
            style={{ width: '100%', padding: '14px 16px', background: 'transparent', border: 'none', fontSize: '14px', color: '#ef4444', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, textAlign: 'left' as const }}>
          ...
          <motion.div
            ...
            <motion.div
              ...
              <div style={{ textAlign: 'center' as const, marginBottom: '24px' }}>
                ...
                <h2 style={{ fontSize: '22px', fontWeight: 800, color: text, letterSpacing: '-0.03em', marginBottom: '6px', fontFamily: "'DM Sans', sans-serif" }}>KiWardrobe Pro</h2>
              ...
              {[
                ...
                { icon: '♡', title: locale === 'de' ? 'Unbegrenzt speichern' : 'Unlimited saved outfits', sub: locale === 'de' ? 'Statt max. 5 Outfits' : 'Instead of max. 5 outfits' },
                ...
                <div key={i} style={{ display: 'flex', gap: '12px', alignItems: 'center', padding: '10px 0', borderBottom: i < 3 ? `1px solid ${border}` : 'none' }}>
                  ...
                  <div>
                    <p style={{ fontSize: '14px', fontWeight: 700, color: text, marginBottom: '2px', fontFamily: "'DM Sans', sans-serif" }}>{f.title}</p>
