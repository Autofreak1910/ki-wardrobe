...
const categoryInstruction = 'category must be exactly one of these English words: top, pants, shorts, skirt, dress, jacket, shoes, accessory. Always use the English word for the "category" field specifically, regardless of what language you respond in for everything else. "top" = shirts/t-shirts/sweaters/hoodies. "pants" = long pants/jeans. "shorts" = shorts/bermudas. "skirt" = skirts of any length. "dress" = dresses/jumpsuits/overalls. "jacket" = jackets/coats/blazers. "shoes" = any footwear. "accessory" = bags/belts/hats/jewelry.'

    const prompt = locale === 'de'
      ? `Du siehst ein Foto eines Kleiderschranks oder mehrerer Kleidungsstücke. Identifiziere JEDES einzelne, klar erkennbare Kleidungsstück. Gib für jedes Teil eine großzügige Bounding Box in Prozent (x, y = obere linke Ecke, width, height), Kategorie, Farbe, Namen und Marke falls erkennbar. ${categoryInstruction} WICHTIG: "color" und "name" IMMER auf Deutsch beantworten (z.B. "Grau", "Grauer Hoodie"), egal welche Sprache auf dem Kleidungsstück selbst steht. Lieber etwas zu groß als zu klein. Ignoriere verdeckte oder unscharfe Teile. Antworte NUR mit JSON: {"items": [{"x": 5, "y": 8, "width": 25, "height": 60, "category": "top", "color": "Grau", "name": "Grauer Hoodie", "brand": null}]}`
      : `You see a photo of a closet or multiple clothing items. Identify EVERY clearly visible clothing item. Give a generous bounding box in percent (x, y = top-left, width, height), category, color, name and brand if visible. ${categoryInstruction} IMPORTANT: always respond with "color" and "name" in English, regardless of what language is printed on the garment itself. Err on the side of too large rather than too small. Ignore obscured or blurry items. Respond ONLY with JSON: {"items": [{"x": 5, "y": 8, "width": 25, "height": 60, "category": "top", "color": "Gray", "name": "Gray Hoodie", "brand": null}]}`
